...
5. Own source-controlled ecosystem changes through one bounded `ecosystem_recovery` plan. The recovery coordinator may edit only the ecosystem repository. After complete validation, the trusted host may create one local commit containing only the verified repair; it must never include a pre-existing dirty baseline or push. Never edit product code, loosen permissions, reveal credentials, delete history, or perform external writes. Route product source changes to Developer.
