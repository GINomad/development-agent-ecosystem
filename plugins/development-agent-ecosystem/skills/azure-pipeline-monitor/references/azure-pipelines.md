...
|---|---|---|---|
| `814` | `example-app` | Standard UI/.NET build and Web App artifact | `main` only |
| `892` | `example-app` | Docker build and ACR push | None; manual |
| `891` | `example-infrastructure` | Excel Agent infrastructure/container deployment | None; manual |
...
The agent may automatically queue build definitions `814` then `892` after a `example-app` push. Definition 814 must succeed for the exact commit before a new 892 is queued and accepted. Never automatically queue `891`; confirm deployment intent and parameters first. For the current DEV instance, the established values are `Dev`, `001`, and the successful run ID from definition `892` as `imageTag`.
