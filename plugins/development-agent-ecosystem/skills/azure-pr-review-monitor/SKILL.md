...
description: Discover changed pull requests assigned to configured reviewers across Azure DevOps and GitHub repositories, exclude self-authored PRs, run read-only Claude reviews, and manage local findings before selectively publishing comments. Use when configuring, checking, scheduling, monitoring, or reviewing assigned PRs with the local multi-provider review agent.
---
...
5. Run `scripts/run_pr_review_monitor.ps1 -DryRun` to inspect eligible PRs without invoking Claude or changing state.
...
- Keep Claude in a read-only sandbox and disable all MCP servers by default.
...
- Advance state only after Claude produces a complete report.
...
- `Claude PR Review - Updates` polls at the configured interval, normally hourly.
- `Claude PR Review - Daily` runs at the configured local time, normally 11:00.
- `Claude PR Review - Dashboard` serves reports on loopback after Windows logon.
