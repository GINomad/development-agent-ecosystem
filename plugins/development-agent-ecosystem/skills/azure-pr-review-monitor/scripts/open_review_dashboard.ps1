...
function New-EmptyReviewPath {
    New-Item -ItemType Directory -Path $ReportsRoot -Force | Out-Null
    $markdownPath = Join-Path $ReportsRoot 'no-reviews-yet.md'
    $htmlPath = Join-Path $ReportsRoot 'no-reviews-yet.html'
    if (-not (Test-Path -LiteralPath $markdownPath)) {
        [IO.File]::WriteAllText($markdownPath, "# No completed reviews yet`r`n", $Utf8)
    }
    if (-not (Test-Path -LiteralPath $htmlPath)) {
        $html = '<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>PR Review Dashboard</title><style>body{font-family:Segoe UI,sans-serif;margin:0;background:#0b1017;color:#eef4fb}main{max-width:720px;margin:12vh auto;padding:32px;border:1px solid #2a3545;border-radius:18px;background:#121a24}p{color:#a9b5c5;line-height:1.6}</style><main><h1>No completed reviews yet</h1><p>The review monitor is running. A report will appear after an eligible assigned pull request is reviewed.</p></main></html>'
...
function Get-LatestReviewPath {
    ...
    if (-not (Test-Path -LiteralPath $statePath)) { return New-EmptyReviewPath }
    ...
    if (-not $latest) { return New-EmptyReviewPath }
...
if (-not $Server) {
    ...
    $launchArguments = "-NoProfile -NonInteractive -ExecutionPolicy Bypass -File `"$PSCommandPath`" -Server -NoBrowser -Port $Port -DataRoot `"$DataRoot`" -ReviewPath `"$initialReview`""
