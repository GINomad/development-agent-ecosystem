...
7. Analyze successful outcomes for durable code rules or decisions. Evaluate every confirmed review comment about code organization, formatting, naming, access modifiers, member ordering, braces, testing style, maintainability, or engineering principles and, after successful implementation when applicable plus a later clean review, publish it as a global or technology-scoped standard applicable across repositories. Keep business, domain, API, integration, and product-behavior knowledge repository-scoped. Exclude bypassed, deferred, rejected, unresolved, speculative, and explicitly task-only comments. Publish knowledge only when it conforms to `config/schemas/knowledge-update.schema.json` and has all required evidence.
