...
An evidence-first local Claude Code ecosystem for the complete software delivery cycle: requirements analysis, knowledge management, implementation, candidate code review, independent review verification, and Azure Pipelines monitoring.

The canonical configuration is [`config/agents.json`](config/agents.json). Every workflow start reloads and validates this JSON file, then compiles it into native Claude Code plugin-agent Markdown definitions. Do not edit generated agent files manually.
...
| Knowledge Keeper | Pull-based knowledge and skill service, final outcomes, task summary, and verified knowledge updates with a documentation-style human-readable presentation | Never routes routine comments, polls subagents, ingests unfinished context, or presents proposed knowledge as user guidance |
...
See [Claude Code setup](docs/claude-code.md), [installation](docs/installation.md), [architecture](docs/architecture.md), [configuration](docs/configuration.md), [pipeline monitoring and ownership](docs/pipeline-monitoring.md), and [operations and rollback](docs/operations.md).
...
```powershell
# Terminal 1
.\scripts\Start-DevelopmentWorkflow.ps1 -Mode manual -TaskSelector 1839566 -RepositoryId azure-example-app

# Terminal 2
.\scripts\Start-DevelopmentWorkflow.ps1 -Mode manual -TaskSelector 1839567 -RepositoryId azure-example-app
```
...
.\scripts\Start-DevelopmentWorkflow.ps1 -Mode manual -TaskSelector 1839566 -RepositoryIds azure-example-app,azure-example-infrastructure
...
.\scripts\Invoke-PostPushPipeline.ps1 -TaskId task-1839566 -RepositoryId azure-example-app -PushWasSuccessful -Branch feature/example -Commit <full-40-character-sha> -QueuedAfter <pre-push-utc>

# Preview the guarded non-force working-branch push without writing remotely
.\scripts\Invoke-ReviewedBranchDelivery.ps1 -TaskId task-1839566 -RepositoryId azure-example-app -PrepareOnly
...
- The configured reviewed-branch delivery capability is standing authorization for a normal push of the exact persisted `features/<task-name>` or `bugfix/<bug-name>` branch only after a clean review and a passing independent verification bound to that exact review SHA, or after every independently confirmed remaining product finding is explicitly rejected or bypassed into an exact-review-bound open task-local debt item. It rejects stale/failed verification, blocked coverage, `main`/`master`, internal `agent/*` branches, branch/manifest mismatches, dirty worktrees, force, tags, approved/deferred/undecided confirmed findings, and untracked bypasses. Review-comment publication, deployments, and work-item mutation remain separately gated. Build queueing stays limited to `pipeline.repositories[].autoQueueDefinitionIds`; build 892 is allowed and deployment 891 is rejected.
...
The optional local ecosystem-read MCP server provides task-scoped, read-only state, public JSON artifacts, and a bounded ledger tail. Role policies are deny-by-default; external MCP servers must be explicitly registered as read-only and selected per role during setup. An Azure server can provide bounded related work items, comments, and history to Requirements Analyst, which publishes revision-bound evidence for review. If a server is unhealthy, the workflow circuit opens and new runs use the existing classic context-pack path while Health Check probes recovery. See [configuration](docs/configuration.md), [the Azure adapter contract](docs/mcp-azure-adapter-contract.md), and `scripts/Get-McpMigrationReport.ps1` for baseline/canary gates.
