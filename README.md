...
- The configured Health verified-repair delivery capability is standing authorization only for the trusted host after full ecosystem validation and a repair-only commit from a clean baseline. It validates the exact ecosystem `origin` URL, avoids base-branch pushes by creating a deterministic repair branch, forbids force and tags, and verifies the exact remote SHA. It does not authorize product repositories or any other external action.
