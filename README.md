...
| Reviewer | Reviews code and Developer-agent work against requirements, held scope, knowledge, and tests; owns task-local debt records for explicitly bypassed findings | Source-code read-only; a finding is not automatic permission to make a change, and bypass is not resolution |
...
- The configured reviewed-branch delivery capability is standing authorization for a normal `origin/<working-branch>` push only after a clean local review or after every remaining product finding is explicitly rejected or bypassed into a linked open task-local debt item. It rejects `main`/`master`, dirty worktrees, force, tags, approved/deferred/undecided findings, and untracked bypasses. Review-comment publication, deployments, and work-item mutation remain separately gated. Build queueing stays limited to `pipeline.repositories[].autoQueueDefinitionIds`; build 892 is allowed and deployment 891 is rejected.
- Pipeline responsibility is configuration-driven: `pipeline_monitor` observes every configured exact SHA; `developer` fixes supported product code/test/YAML failures; `reviewer` reviews the remediation; `orchestrator` routes exceptions and validates completion; `health_check` owns ecosystem defects; `knowledge_keeper` publishes the final verified outcome.
