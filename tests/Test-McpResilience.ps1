...
Assert-That ($workflowSource -match 'Expand-EcosystemValue -Value \$value.+-StateRoot \$mcpStateRoot') 'Workflow does not expand MCP command arguments before passing them to Claude Code.'
Assert-That ($workflowSource -notmatch '@\(\$mcpServer\.arguments \| ForEach-Object \{ \[string\]\$_ \}\) \| ConvertTo-Json') 'Workflow still passes unresolved MCP command arguments to Claude Code.'
...
$prMonitorSource=Get-Content -LiteralPath (Join-Path $root 'plugins\development-agent-ecosystem\skills\azure-pr-review-monitor\scripts\run_pr_review_monitor.ps1') -Raw -Encoding UTF8
Assert-That ($workflowSource -match "'--strict-mcp-config'" -and $workflowSource -match "'--mcp-config'" -and $workflowSource -match "'--allowedTools'") 'Workflow does not constrain Claude MCP access to the role allowlist.'
Assert-That ($prMonitorSource -match 'Assert-ClaudeReviewMcpPolicy' -and $prMonitorSource -match '--strict-mcp-config' -and $prMonitorSource -match '--tools') 'Claude PR review monitor does not fail closed with tools and MCP disabled.'
