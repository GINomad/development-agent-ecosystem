...
- **Resume workflow elevated** explicitly applies the same checkpoint plan without the Claude Code OS sandbox; all delivery gates remain active.
...
.\scripts\Resolve-TaskWorkspace.ps1 -TaskId task-1839566 `
  -RepositoryId azure-example-app -AllowReleased |
  Format-List

# Global lease registry (read-only inspection)
Get-Content "$env:LOCALAPPDATA\Claude\development-agent-ecosystem\workspace-coordinator.json" -Raw |
...
The canonical owners and current definition IDs are listed in [pipeline monitoring and ownership](pipeline-monitoring.md). In short, Pipeline Monitor owns all configured exact-SHA observation; Developer, Reviewer, and Review Verifier own the bounded product remediation loop; Orchestrator owns exceptional and terminal routing; Health Check owns ecosystem defects; Knowledge Keeper owns final publication.
...
The watcher discovers only runs whose full `sourceVersion` equals the pushed SHA. If canonical JSON contains an ordered approved build sequence, it queues each definition only after the previous definition succeeds. For `example-app`, the standing sequence is 814 then a new 892; an earlier 892 is ignored for acceptance. Deployment 891 is never auto-queued; `example-infrastructure` and `example-service` currently have empty auto-queue lists.

Failed task logs are reduced to the configured tail and byte limit, then classified without an AI call:

- `code` and `test`: write a deduplicated `pipeline-remediation-<signature>.json`, set Developer, Reviewer, Review Verifier, and Pipeline Monitor back to `pending`, and append a targeted `pipeline-remediation-request`; this preserves prior artifacts while ensuring the new commit is reviewed, independently verified, and monitored;
...
Developer fixes only supported code/test evidence, runs local validation, and sends the change through Reviewer and then the independent Review Verifier. A later push still requires explicit authorization and a passing verification bound to the exact current review SHA. After that push, Pipeline Monitor validates the new exact SHA; completed unrelated agents and artifacts are not restarted.
...
.\scripts\Invoke-PostPushPipeline.ps1 `
  -TaskId task-1839566 `
  -RepositoryId azure-example-app `
  -PushWasSuccessful `
...
Ecosystem maintenance is physically isolated from product repositories. When Orchestrator selects `ecosystem-repair`, the trusted workflow launcher automatically uses the development-agent-ecosystem repository as both the Claude Code working directory and the child-process working directory. Product task clones remain read-only context and are not passed as writable additional directories; only task runtime state is added for outcome publication. A copied launcher still fails before execution when its own ecosystem root equals or is contained by a configured product `repositories[].localWorkspace`. Ecosystem code, configuration, prompts, skills, tests, documentation, temporary files, and recovery artifacts must never be created or modified beneath a product repository.
...
The local task review diff includes the complete persisted Reviewer summary, product findings, held-scope violations, agent-process suggestions, requirement-to-code traceability, all ten review-coverage dimensions, independent coverage verdicts, and finding lifecycle across revisions. The code pane has its own vertical and horizontal scroll. Select a diff row to reveal the comment editor directly below that row; click the same selected row again to close it, or click another row to move it. No editor overlays an unselected file. Source-specific Reviewer findings are rendered inline at their structured `codeLocation`; cards also show `new`, `unchanged`, `resolved`, or `regressed` plus `confirmed`, `rejected`, or `needs-human`. Requirement Traceability records one entry per analyzed requirement and exposes exact repository-relative files and one-based line ranges; selecting a reference navigates to that diff line when it is part of the current patch. Each finding has a durable reply thread backed by `task-ledger.jsonl`. **Send to Reviewer** requests clarification or correction from Reviewer; **Send to Developer** is disabled for verifier-rejected findings and otherwise queues the same item as implementation input without authorizing it. Neither action restarts an agent or approves a finding. Use the separate restart and review-decision controls when those actions are intended.

Reviewer records candidates but does not authorize changes. Review Verifier independently tests the complete matrix, findings, and lifecycle in a separate read-only invocation. Human decisions are accepted only for `confirmed` or `needs-human` findings in a passing `review-verification.json` bound to the exact current review SHA. Record the decision separately:
...
Allowed decisions are `approved`, `rejected`, `deferred`, and `bypassed`. Developer must not apply a finding without an `approved` exact-review-bound decision. `deferred` remains blocking. `bypassed` requires a non-empty reason and atomically creates or reuses `TD-REV-NNN` in task-local `tech-debt-items.json`; both decision and debt record retain the reviewed revision, review SHA, and verification evidence. The finding remains unresolved but Pipeline Monitor may continue. A stale verification, verifier-rejected finding, blocked coverage dimension, or missing/closed/mismatched debt item fails the gate.
...
Rollback disables the new tasks and enables the legacy `Claude Code PR Review - ...` tasks. It does not remove the new plugin, its isolated `DataRoot`, or either global monitor skill, so diagnostics and forward migration remain available.
