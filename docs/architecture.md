...
`task.json` is a current-state projection used for fast dashboard rendering. `task-ledger.jsonl` remains the durable history. The dashboard polls the loopback API every five seconds, while Knowledge Keeper rereads unacknowledged user comments at every handoff checkpoint. Failed agents provide structured evidence immediately; Health Check status is persisted and rendered like every other agent. Automatic recovery is limited to one attempt per failure signature in the ecosystem repository and cannot touch product code or external systems. Comments and recovery never bypass unresolved-requirement, review-approval, or external-write gates.
