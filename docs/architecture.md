...
| `scripts/AgentEcosystem.psm1` | JSON loading, semantic validation, path expansion, and Claude agent Markdown generation primitives |
...
| `scripts/Invoke-GuardedAgentRuntime.ps1` | Native Claude Code supervision, UTF-8 log normalization, three-identical-failure cutoff, and execution-guard artifacts |
...
| `%LOCALAPPDATA%/Claude/development-agent-ecosystem` | Mutable task ledgers, review prompts, notes, reports, and scheduler backups |
...
| `+ SKILL.MD` | Add a plugin skill directory containing `SKILL.md` and `Claude plugin metadata`, then reference it from `skillPaths[]` |
...
| `+ COMPILE STEPS` | Extend Claude agent Markdown generation while keeping `config/agents.json` canonical |
...
`queued` tasks have no controller lease. The coordinator admits the oldest eligible queued task only after capacity is available. `Released` preserves the full clone for ordinary resume while the task remains open. Once final closure, the completed task summary, and lease release are all durable, cleanup deletes only the task clone and marks its manifest `cleaned`; task artifacts and history remain, and explicit reopen provisions a new clone. A failed first-time provision removes only its own incomplete clone and lease.
...
Runtime task history is stored outside the repository under `%LOCALAPPDATA%/Claude/development-agent-ecosystem/tasks/<task-id>`:
