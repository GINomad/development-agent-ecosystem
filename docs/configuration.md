...
`workflow.automaticContinuation` controls host-driven next-link execution after initial, resume, and targeted runs. `maxChainSteps` is a hard per-continuation bound, and `maxTransitionRepeats` stops the fourth occurrence of the same role-to-role transition and hands the failure to Health Check. `useElevatedExecution` selects the host-compatible profiles on this machine. Human-input and approval gates stop the chain unless the completed role produced an explicit machine-readable handoff that owns that gate, such as Developer to Reviewer or Pipeline remediation to Developer.
