...
`health` controls automatic failure handling. `repairMode` is fixed to `safe-deterministic-only`. `automaticRecovery.workspace` must resolve to this ecosystem repository; `allowEcosystemSourceChanges=true` permits the bounded recovery coordinator to edit it, while product-code changes and model-driven external writes remain permanently disabled. After complete validation, `commitVerifiedRepairs=true` creates one repair-only commit from a previously clean worktree. `pushVerifiedRepairs=true` is the narrow trusted-host exception: it permits a normal non-force, non-tag push only to the exact `pushRemote=origin` and `pushRemoteUrl`, only on a non-base branch, followed by exact remote-SHA verification. If recovery started on `main` or `master`, the host first creates a deterministic branch under `repairBranchPrefix`. It never includes a pre-existing dirty baseline or authorizes product, PR, pipeline, deployment, or work-item writes. `maxAttemptsPerFailureSignature` prevents recovery loops and defaults to one. Automatic recovery always uses `workspace-write`. `elevatedFallback` permits one `danger-full-access` attempt only after an explicit confirmation in the local dashboard; this is intended solely for Windows process-creation error 1260.
