...
`workflow.automaticContinuation` controls host-driven next-link execution after initial, resume, and targeted runs. `maxChainSteps` is a hard per-continuation bound (16 by default), and `maxTransitionRepeats` allows four occurrences of the same role-to-role transition so the initial delivery pass and all three permitted pipeline remediation cycles can complete; a fifth occurrence fails closed to Health Check. `useElevatedExecution` selects the host-compatible profiles on this machine. `recoveryGraceSeconds` prevents recovery from racing a healthy host, while `recoveryPollIntervalMinutes` controls the deterministic resident reconciler. The hidden at-logon host reloads this JSON after every pass, so interval changes need no task reinstall or host restart. Developer may cross `review_pending` only to Reviewer; Reviewer crosses it only to Review Verifier. The verifier may return rejected coverage/lifecycle claims to Reviewer, stop at blocked coverage or human decisions, or continue a verified clean outcome. Pipeline remediation may cross its waiting gate only to Developer.
...
`workflow.workspaceScheduling` controls capacity for independent task clones. `maxActiveTasks` is at least two (two by default), `queueWhenBusy` keeps overflow work in a durable FIFO `queued` state, and `maxActiveAgentsPerTask=1` enforces one controller chain per task. `workspaceRoot` contains one full clone per `(taskId, repositoryId)`. `coordinatorStatePath` stores active task leases with exact run/lease IDs, while each clone has a manifest containing its absolute path, canonical origin, base SHA, unique task branch, lifecycle, run ID, and lease ID. Task metadata persists `features/<task-name>` for implementation work and `bugfix/<bug-name>` for bugs. A legacy resume backfills missing metadata and migrates an `agent/*` manifest and checkout to that branch at the unchanged HEAD before returning the clone. Releasing a lease does not itself delete or reset the clone, so an explicit resume preserves uncommitted task work while the task remains open. Final summarized closure triggers guarded clone deletion and a later reopen provisions a new clone. `leaseHeartbeatSeconds` controls exact task/run/lease renewal by a live runner. `staleLeaseGraceSeconds` bounds missed-heartbeat recovery, including a crashed in-process runspace while the shared dashboard PID remains alive; terminal task leases are recoverable immediately. Recovery marks a non-terminal orphaned task `interrupted`, records an event, and preserves its clone. `lockTimeoutSeconds` bounds coordinator and per-task state locks. Runtime consumers resolve only the manifest path; they never execute in `repositories[].localWorkspace`, use Git worktrees, or stash/switch another task's checkout.
...
`pipeline.repositories[]` maps an ecosystem repository ID to optional monitored `definitionIds` and the standing ordered `autoQueueDefinitionIds` allowlist. An empty definition list discovers all exact-SHA runs. The current `example-app` entry queues 814 and, only after its success, a new 892 for the pushed SHA. An earlier 892 does not satisfy the sequence. Deployment 891 is rejected by semantic validation and cannot be added to an auto-queue list. The other repositories currently monitor triggered exact-SHA runs but do not auto-queue a definition.
...
`runtime.elevatedFallback` governs only the trusted PowerShell host compatibility path retained from the shared control plane. Claude Code always uses the canonical generated Markdown agents and committed `auto` permission mode; it does not compile or select suffixed runtime agents.

`runtime.elevatedFallback.launchStrategy` is fixed to `in-process-runspace`. The dashboard does not create a nested encoded PowerShell process for the confirmed workflow, because enterprise endpoint policy may deny that parent-child pattern before Claude Code starts. Completed runspaces are disposed and recorded in `%LOCALAPPDATA%/Claude/development-agent-ecosystem/dashboard-runspaces.jsonl`.
...
3. compiles agent prompts and skills into Claude Code Markdown;
4. installs the updated Claude agent Markdown files into `runtime.claude.agentInstallRoot`;
5. creates or resumes the task ledger.

To change agent behavior, edit the canonical JSON, a prompt, or a repository skill. Generated Claude agent Markdown files are not a source of truth.
...
Each `agents[]` entry also contains a `responsibilities[]` directory consumed by Orchestrator at every workflow start. Update that list whenever a role's authority changes. Add prompts through `agents[].promptPaths` and skills through `agents[].skillPaths`. Paths may use `${REPO_ROOT}`, `${CLAUDE_HOME}`, `${STATE_ROOT}`, and `${LOCALAPPDATA}`. The legacy `${CODEX_HOME}` token remains accepted only for configuration migration. Every skill must contain a valid `SKILL.md` file and Claude plugin metadata.
