...
|---|---|---|---|---|
| `azure-example-app` | `example / ExampleProject / example-app` | Exact-SHA runs; an empty `definitionIds` list permits discovery | `814`, then a newly queued `892` only after 814 succeeds | `pipeline_monitor` |
| `azure-example-infrastructure` | `example / ExampleProject / example-infrastructure` | Triggered exact-SHA runs; empty `definitionIds` means discovery is not restricted to one definition | None; observation only | `pipeline_monitor` |
| `azure-example-service` | `palantir-consulting / ExampleProject / example-service` | Definition `17` for the exact SHA | None; definition 17 is observation only | `pipeline_monitor` |
