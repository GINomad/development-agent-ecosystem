...
function Invoke-Attempt {
    ...
    & (Join-Path $PSScriptRoot 'Invoke-GuardedAgentRuntime.ps1') @attemptParameters
}
...
if ($canFallback) {
    ...
    $effortFlagIndex = [array]::IndexOf($fallbackArguments, '--effort')
    if ($effortFlagIndex -ge 0) {
        $fallbackArguments[$effortFlagIndex + 1] = $FallbackReasoningEffort
    }
    else {
