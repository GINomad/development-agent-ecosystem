...
if ($taskDiffScript -notmatch "ValidateSet\('reviewed-commit','all-task-changes'\)" -or $taskDiffScript -notmatch "reviewedRevision -match '\^git:" -or $taskDiffScript -notmatch '\$diffTarget\^' -or $dashboardClient -notmatch "reviewDiffScope = 'reviewed-commit'" -or $dashboardClient -notmatch 'all-task-changes' -or $dashboardServer -notmatch "Diff scope is not supported") { throw 'Reviewer diff must default to the exact reviewed commit against its first parent and retain an all-task-changes option.' }
